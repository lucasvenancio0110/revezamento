(()=>{'use strict';
const END=24*60,MOVE=10,MEAL=60;
const norm=s=>String(s||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toUpperCase().replace(/[^A-Z0-9]+/g,' ').trim();
const aliases={
  'LUCAS':'Lucas V.','LUCAS V':'Lucas V.','LUCAS VENANCIO':'Lucas V.',
  'LUCAS R':'Lucas R.','MARCIO':'Marcio','MARCIO ':'Marcio,
  'EWERSON':'Ewerson','EVERSON':'Everson','PATRICIO':'Patrício','CHRISTOFFER':'Christoffer'
};
...
function interval(ev){
  const s=toMin(ev.start||ev.time),type=ev.type||ev.kind;
  if(s==null)return null;
  let e=toMin(ev.end);
  if(e==null)e=type==='future'||type==='setup-future'?END:s+(Number(ev.duration)||MEAL);
  return {s,e};
}
...
function collectAssignments(plan){
  const by=new Map();
  const add=(person,ev)=>{if(!person)return;const p=canon(person);if(!by.has(p))by.set(p,[]);by.get(p).push(ev)};
  plan.meals.forEach(m=>add(m.person,m));
  plan.covers.forEach(c=>add(c.person,c));
  plan.tasks.forEach(t=>add(t.assignee||t.owner,t));
  return by;
}
function validate(input){
  ...
  for(const [person,events] of by){
    ...
    for(let i=0;i<valid.length;i++)for(let j=i+1;j<valid.length;j++){
      const a=valid[i],b=valid[j];if(overlap(a.i.s,a.i.e,b.i.s,b.i.e))errors.push({code:'PERSON_OVERLAP',person,message:`${person} tem conflito entre ${label(a.e)} e ${label(b.e)}.`,events:[a.e.id,b.e.id]});
    }
  }
...
function score(input){
  ...
  const assigned=p.tasks.filter(t=>['future','setup-future'].includes(t.type)&&t.assignee).length;
  const total=p.tasks.filter(t=>['future','setup-future'].includes(t.type)).length;
  const coverage=total?Math.round(assigned/total*100):100;
  const dinners=p.people.length?Math.round((p.people.length-r.warnings.filter(w=>w.code==='MEAL_MISSING').length)/p.people.length*100):100;
...
function candidates(plan,task){
  const start=toMin(task.start||task.time);if(start==null)return[];
  const end=['future','setup-future'].includes(task.type)?END:start+(task.duration||MEAL);
  const by=collectAssignments(plan);
  return plan.people.map(p=>p.name).filter(person=>{
    const events=by.get(person)||[];
    return !events.some(e=>{if(e.id===task.id)return false;const i=interval(e);return i&&overlap(i.s,i.e,start,end)});
  });
}
function optimize(input,{limit=5000}={}){
  const base=normalizePlan(input),future=base.tasks.filter(t=>['future','setup-future'].includes(t.type)).sort((a,b)=>toMin(a.start||a.time)-toMin(b.start||b.time));
  let explored=0,best=null;
  const walk=(index,plan)=>{
    if(explored++>=limit)return;
    if(index>=future.length){const s=score(plan);if(!best||s.score>best.metrics.score)best={plan:structuredClone(plan),metrics:s};return}
    const current=plan.tasks.find(t=>t.id===future[index].id);if(current.assignee){walk(index+1,plan);return}
    const opts=candidates(plan,current);
    if(!opts.length){walk(index+1,plan);return}
    for(const name of opts){current.assignee=name;walk(index+1,plan);current.assignee=null;}
  };
  walk(0,base);
  if(!best)best={plan:base,metrics:score(base)};
  return {...best,explored,explanation:explain(best.metrics)};
}
function explain(m){
  if(m.valid)return `Plano validado: ${m.coverage}% dos próximos setups cobertos e ${m.dinners}% dos jantares definidos.`;
  const open=m.errors.filter(e=>e.code==='FUTURE_UNASSIGNED').map(e=>e.message);
  if(open.length)return `Plano não validado. ${open.join(' ')}`;
  return `Plano não validado: ${m.errors[0]?.message||'há conflitos operacionais.'}`;
}
window.NCDE={version:'1.0.0',canon,toMin,toTime,normalizePlan,validate,score,optimize,candidates};
window.dispatchEvent(new CustomEvent('ncde:ready',{detail:{version:'1.0.0'}}));
})();